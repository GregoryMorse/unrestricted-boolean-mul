...
- a gate-count-free definition of every finite defect-legal unrestricted
  ...
  gain and the resulting monotonicity of the completion deficit; and
- an intrinsic, algebraic classification of every independent quadratic
...
- the exact canonical zero-defect state `Aff + R`, kernel-reduced fixed-block
  certificates for the rational and degree-two one-defect shadow sets, and
  the corrected rooted colour-family rank bound; and
- gate-by-gate cost-preserving replay from a larger equal-defect state,
  deleting products already present and certifying exact dimension growth
  for every retained gate.  These isolate the remaining load-bearing Lean
  work as dynamic restart after quadratic defect births, the semantic
  rooted-shadow assignment in defects zero and one, and the stable
  two-defect envelope theorem.
