import UnrestrictedBooleanMul.Phase2
import Lean.Elab.Tactic.Omega

/-!
# Four-term Hankel target geometry

This file starts Phase 3 with the coordinate geometry of the seven-dimensional
...
theorem rankOne_target_classification {c : TargetCoeff}
  ...
  · have hc1 : c 1 = 0 := by
      ...
      simp [hc0, Phase2Certificate.mul_self_f2] at hm
      exact hm
    have hc2 : c 2 = 0 := by
    ...
    have hc4 : c 4 = 0 := by
      ...
      simp [hc2, Phase2Certificate.mul_self_f2] at hm
      exact hm
    have hc5 : c 5 = 0 := by
      have hm := rankOne_minor h (2 : Fin 4) 3 3 2
      simp [hc4, Phase2Certificate.mul_self_f2] at hm
      exact hm
    have hc6 : c 6 = 1 := by
  ...
  · have hc2 : c 2 = c 1 := by
      ...
      simpa [hc0, Phase2Certificate.mul_self_f2] using hm
    have hc3 : c 3 = c 1 := by
      have hm := rankOne_minor h (0 : Fin 4) 1 0 2
      simpa [hc0, hc2, Phase2Certificate.mul_self_f2] using hm
    have hc4 : c 4 = c 1 := by
      have hm := rankOne_minor h (0 : Fin 4) 2 0 2
      simpa [hc0, hc2, Phase2Certificate.mul_self_f2] using hm
    have hc5 : c 5 = c 1 := by
      have hm := rankOne_minor h (0 : Fin 4) 2 0 3
      simpa [hc0, hc2, hc3, Phase2Certificate.mul_self_f2] using hm
    have hc6 : c 6 = c 1 := by
      have hm := rankOne_minor h (0 : Fin 4) 3 0 3
      simpa [hc0, hc3, Phase2Certificate.mul_self_f2] using hm
