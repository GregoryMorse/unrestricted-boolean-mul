...
/-- The support of a nonzero decomposable two-form generated by two
independent vectors is exactly their two-dimensional span. -/
theorem quadraticSupport_squarefreeWedge {m : Nat}
    (u v : Fin m → F₂)
    (hlin : LinearIndependent F₂ ![u, v]) :
    quadraticSupport (squarefreeWedge u v) =
      Submodule.span F₂ (Set.range ![u, v]) := by
  let w : Fin 2 → Fin m → F₂ := ![u, v]
  have hw : LinearIndependent F₂ w := hlin
  apply le_antisymm
  · rintro _ ⟨z, rfl⟩
    rw [quadraticContraction_squarefreeWedge]
  ...
  · apply Submodule.span_le.mpr
    rintro _ ⟨i, rfl⟩
    fin_cases i
    · obtain ⟨z, hz⟩ := exists_coordinateDot_dual w hw (1 : Fin 2)
      refine ⟨z, ?_⟩
      rw [quadraticContraction_squarefreeWedge]
      ...
      rw [hzu, hzv]
      simp
    · obtain ⟨z, hz⟩ := exists_coordinateDot_dual w hw (0 : Fin 2)
      refine ⟨z, ?_⟩
      rw [quadraticContraction_squarefreeWedge]
