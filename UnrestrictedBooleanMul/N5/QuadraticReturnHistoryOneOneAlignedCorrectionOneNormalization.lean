import UnrestrictedBooleanMul.N5.QuadraticReturnHistoryOneOneAlignedCorrectionOneRaw
import UnrestrictedBooleanMul.N3Certificate
import UnrestrictedBooleanMul.N5.QuadraticReturnHistoryOneOneAlignedBase

/-!
# Affine normalization bridge: `OneOneAlignedCorrectionOne`
...
/-- Original semantic generators selected by the `OneOneAlignedCorrectionOne` unit certificate. -/
def oneOneAlignedCorrectionOneOriginalConstraint (v : Fin 71 → F₂) : Fin 13 → F₂ :=
...
structure OneOneAlignedCorrectionOneSubstitutions (v : Fin 71 → F₂) : Prop where
  y1 : v 31 = 1
  y2 : v 32 = 0
  y3 : v 33 = v 23
  y4 : v 34 = 0
  ...
  correctionTarget6 : v 68 = 1 + v 1 + v 42
  correctionTarget7 : v 69 = 1 + v 1 + v 4 + v 42 + v 45
  correctionTarget4 : v 66 = v 2 + v 43
  factorLinear2 : v 43 = v 2
  correctionTarget1 : v 63 = 0
  factorLinear4 : v 45 = 1 + v 1 + v 4 + v 42
  factorLinear7 : v 48 = v 7
  factorLinear1 : v 42 = 1 + v 1
...
theorem oneOneAlignedCorrectionOneReducedConstraint_eq_original
    (v : Fin 71 → F₂)
    (h : OneOneAlignedCorrectionOneSubstitutions v)
    (i : Fin 13) :
    oneOneAlignedCorrectionOneReducedConstraint v i = oneOneAlignedCorrectionOneOriginalConstraint v i := by
  fin_cases i <;>
    simp [oneOneAlignedCorrectionOneReducedConstraint, oneOneAlignedCorrectionOneOriginalConstraint,
...
inconsistent. -/
theorem oneOneAlignedCorrectionOne_inconsistent_of_original
    (v : Fin 71 → F₂)
    (hsub : OneOneAlignedCorrectionOneSubstitutions v)
    (hzero : ∀ i : Fin 13, oneOneAlignedCorrectionOneOriginalConstraint v i = 0) :
    False := by
  apply oneOne_aligned_correctionOne_reduced_inconsistent v
  intro i
  rw [oneOneAlignedCorrectionOneReducedConstraint_eq_original v hsub i]
  exact hzero i
