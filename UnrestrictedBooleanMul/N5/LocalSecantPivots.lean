...
/-- If the zero rational place is represented, its evaluation and first-jet
coefficient directions are already killed by the intrinsic displacement
quotient. -/
theorem rationalZeroLocalCoeffSpace_le_localDisplacementCoeffSpace
    (Q : Submodule F₂ QuadraticQuotient)
    (hzero : IsRepresentedPlace Q 0) :
    rationalZeroLocalCoeffSpace ≤ localDisplacementCoeffSpace Q := by
  apply Submodule.span_le.mpr
  rintro _ ⟨i, rfl⟩
  fin_cases i
  ...
  · exact closedPlaceDirection_mem_localDisplacementCoeffSpace Q 2 (by
      simp [IsActiveDisplacementDirection])
  · exact closedPlaceDirection_mem_localDisplacementCoeffSpace Q 3 (by
      simpa [IsActiveDisplacementDirection] using hzero)

/-- If the degree-two place is represented, its evaluation and two norm
directions are already killed by the intrinsic displacement quotient. -/
theorem degreeTwoLocalCoeffSpace_le_localDisplacementCoeffSpace
    (Q : Submodule F₂ QuadraticQuotient)
    (hdegree : IsRepresentedPlace Q 3) :
    degreeTwoLocalCoeffSpace ≤ localDisplacementCoeffSpace Q := by
  apply Submodule.span_le.mpr
  rintro _ ⟨i, rfl⟩
  fin_cases i
  · exact closedPlaceDirection_mem_localDisplacementCoeffSpace Q 0 (by
  ...
  · exact closedPlaceDirection_mem_localDisplacementCoeffSpace Q 2 (by
      simp [IsActiveDisplacementDirection])
  · exact closedPlaceDirection_mem_localDisplacementCoeffSpace Q 6 (by
      simpa [IsActiveDisplacementDirection] using hdegree)
  · exact closedPlaceDirection_mem_localDisplacementCoeffSpace Q 7 (by
      simpa [IsActiveDisplacementDirection] using hdegree)
...
/-- Enlarging a coefficient subspace can only enlarge its image after a
fixed displacement quotient. -/
theorem finrank_range_mkQ_domRestrict_mono
    {V : Type*} [AddCommGroup V] [Module F₂ V] [FiniteDimensional F₂ V]
    (S S' D : Submodule F₂ V) (hSS' : S ≤ S') :
    Module.finrank F₂
        (LinearMap.range ((Submodule.mkQ D).domRestrict S)) ≤
      Module.finrank F₂
        (LinearMap.range ((Submodule.mkQ D).domRestrict S')) := by
  apply Submodule.finrank_mono
  rintro y ⟨x, rfl⟩
  exact ⟨⟨x.1, hSS' x.2⟩, rfl⟩
...
theorem secant_pow_three_f2 (x : F₂) : x ^ 3 = x := by
  rw [show 3 = 2 + 1 by omega, pow_succ, N3Certificate.pow_two_f2,
    N3Certificate.mul_self_f2]

theorem secant_pow_four_f2 (x : F₂) : x ^ 4 = x := by
  rw [show 4 = 3 + 1 by omega, pow_succ, secant_pow_three_f2,
    N3Certificate.mul_self_f2]

theorem secant_five_eq_one_f2 : (5 : F₂) = 1 := by decide
theorem secant_seven_eq_one_f2 : (7 : F₂) = 1 := by decide
theorem secant_three_eq_one_f2 : (3 : F₂) = 1 := by decide
theorem secant_nine_eq_one_f2 : (9 : F₂) = 1 := by decide
theorem secant_ten_eq_zero_f2 : (10 : F₂) = 0 := by decide
theorem secant_twelve_eq_zero_f2 : (12 : F₂) = 0 := by decide
theorem secant_thirteen_eq_one_f2 : (13 : F₂) = 1 := by decide
theorem secant_fourteen_eq_zero_f2 : (14 : F₂) = 0 := by decide
theorem secant_sixteen_eq_zero_f2 : (16 : F₂) = 0 := by decide
theorem secant_eighteen_eq_zero_f2 : (18 : F₂) = 0 := by decide
theorem secant_twenty_eq_zero_f2 : (20 : F₂) = 0 := by decide
theorem secant_twenty_two_eq_zero_f2 : (22 : F₂) = 0 := by decide
theorem secant_twenty_four_eq_zero_f2 : (24 : F₂) = 0 := by decide
theorem secant_twenty_six_eq_zero_f2 : (26 : F₂) = 0 := by decide
theorem secant_twenty_eight_eq_zero_f2 : (28 : F₂) = 0 := by decide
theorem secant_thirty_eq_zero_f2 : (30 : F₂) = 0 := by decide
theorem secant_thirty_two_eq_zero_f2 : (32 : F₂) = 0 := by decide
theorem secant_thirty_six_eq_zero_f2 : (36 : F₂) = 0 := by decide
theorem secant_forty_eq_zero_f2 : (40 : F₂) = 0 := by decide
theorem secant_forty_four_eq_zero_f2 : (44 : F₂) = 0 := by decide
theorem secant_fifty_two_eq_zero_f2 : (52 : F₂) = 0 := by decide
