...
static bool has_high(V256 const& f){V256 a=tt_to_anf_slow(f);for(int m=0;m<256;m++)if(bit(a,m)&&__builtin_popcount((unsigned)m)>=3)return true;return false;}
...
int main(int argc,char**argv){
    ...
    if(argc<2){cerr<<"usage: "<<argv[0]<<" SEEDPOS [START [COUNT]]\n";return 2;}
    int seedpos=atoi(argv[1]); if(seedpos<2||seedpos>4){cerr<<"bad seedpos\n";return 2;}
    long long start=(argc>2?atoll(argv[2]):0);
    long long count=(argc>3?atoll(argv[3]):-1);
    auto maps=symmetry_maps();
    // Full PGL2(F2) is transitive on singleton and pair subsets of P^1(F2),
    // so prefix 0 represents all all-useful prefixes at seed positions 2 and 3.
    auto prefixes=prefix_states(seedpos-1); Basis P=prefixes[0];
    auto raw=seed_residuals(P); auto seeds=canonical_seed_orbits(P,raw,maps);
    cerr<<"seedpos="<<seedpos<<" raw_seeds="<<raw.size()<<" orbit_reps="<<seeds.size()<<"\n";
    long long end=(count<0?(long long)seeds.size():min<long long>((long long)seeds.size(),start+count));
    start=max<long long>(0,min<long long>(start,seeds.size())); if(end<start)end=start;
    auto t0=chrono::steady_clock::now(); long long withchild=0; size_t maxch=0;
    for(long long i=start;i<end;i++){
        if(!has_high(seeds[(size_t)i])) continue;
        Basis V=P;V.insert(seeds[(size_t)i]);
        auto ch=useful_children(V); if(!ch.empty()){withchild++;maxch=max(maxch,ch.size()); if(withchild<=20) cerr<<"HIGH_CHILD rep="<<i<<" count="<<ch.size()<<" high="<<high_mons(seeds[(size_t)i])<<"\n";}
    }
    double sec=chrono::duration<double>(chrono::steady_clock::now()-t0).count();
    cout<<"IMMEDIATE seedpos="<<seedpos<<" orbit_reps="<<seeds.size()<<" range=["<<start<<","<<end<<") withchild="<<withchild<<" maxch="<<maxch<<" seconds="<<fixed<<setprecision(3)<<sec<<"\n";
    return 0;
}
