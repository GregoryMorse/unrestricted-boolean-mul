...
| `quadratic_return_history_polynomial.py` | Constructs the same equations as sparse Boolean polynomials, searches low-degree span certificates, emits Singular input, and independently replays the two aligned exceptional lift certificates for `(0,1)` and `(1,1)`. | Python standard library; optional Singular 4.4.1 for certificate discovery | Exact algebraic certificate ledger recorded in `quadratic_return_history_results.txt`.  The stored lift multipliers replay to `1` in an independent Boolean-polynomial implementation. |
