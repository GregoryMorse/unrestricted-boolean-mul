...
| `quadratic_return_population_probe.py` | Tests the post-high same-plane return step that invalidates an unrestricted decomposable restart.  It checks the cubic/quartic cancellation identities and the minimum alternating rank in every Hankel-target coset. | `w_pq_analysis.py` | Quick deterministic scan; all 35 rational-base planes checked. |
| `target_tight_suffix_probe.py` | Finds every target-tight one-gate extension of a supplied fixed state by reducing the multiplication map for one factor at a time; it does not enumerate factor pairs. | Python standard library | Exact for each supplied state; fixed-state discovery only. |
| `cryptominisat_flag_probe.py` | Bounded normalized-flag SAT probe with independent truth-table replay of every reported witness.  The `qreturnce` base is the canonical unpopulated-return regression state. | `pycryptosat` | Falsification/discovery only; an UNSAT result is not a proof premise. |
