...
| `quadratic_return_history_polynomial.py` | Constructs the same equations as sparse Boolean polynomials, searches low-degree span certificates, emits Lean/Singular certificates, independently replays the aligned exceptional lift certificates, and can lift the `(0,1)` exceptional certificates back through affine elimination to the original ideal. | Python standard library; optional Singular 4.4.1 for certificate discovery | Exact algebraic certificate ledger recorded in `quadratic_return_history_results.txt`.  The stored lift multipliers and the mechanically lifted original-ideal identities replay to `1` in an independent Boolean-polynomial implementation. |
