...
| `quadratic_return_class_sample.py` | Falsifies the proposed classification by four fixed returned sections and records that its first 64 ordered samples happen to have trivial target exterior kernel.  The sample is biased and does not support a uniform theorem. | `quadratic_return_population_probe.py`, `w_pq_analysis.py` | Quick bounded regression/discovery scan; never a theorem premise. |
| `quadratic_return_kernel_counterexample.py` | Gives an exact equal-high `(0,r0)` return with nonzero high part, minimum target-coset rank four, and nonzero rational exterior annihilator `r0`. | `quadratic_return_population_probe.py`, `w_pq_analysis.py` | Exact finite-coordinate counterexample to uniform return-kernel sterility; no circuit enumeration. |
| `quadratic_return_feedback_probe.py` | Retains the actual high representative in that counterexample and tests every one of the 2,048 affine-plus-`r0` lower feedback factors against the full returned quadratic correction space. | the return probes above | Exact for the displayed history: no missing-target escape exists.  This is evidence for a history-sensitive theorem, not a universal proof. |
| `quadratic_return_history_sat.py` | Encodes the parameterized equal-high return, retained first high product, returned-section correction, rational feedback factor, and missing-target escape as local Boolean algebra. | `pycryptosat`, the return probes above | All twelve factor-pair/direction cases are UNSAT; the two direct timeouts are related by the fixed-pair rational-place symmetry to completed cases.  Positive controls replay independently.  Discovery only until translated to Lean. |
| `quadratic_return_history_polynomial.py` | Constructs the same equations as sparse Boolean polynomials, searches low-degree span certificates, emits Lean/Singular certificates, independently replays the aligned exceptional lift certificates, and can lift exceptional certificates back through affine elimination to the original ideal. | Python standard library; optional Singular 4.4.1 for certificate discovery | Exact algebraic certificate ledger recorded in `quadratic_return_history_results.txt`.  The stored lift multipliers and all 24 mechanically rediscovered exceptional original-ideal identities replay to `1` in an independent Boolean-polynomial implementation. |
...
| `cryptominisat_chronology_scan.py` | Parallel driver for the normalized-flag probe over every defect chronology. | `cryptominisat_flag_probe.py`, `pycryptosat` | A 60-second scan of all 220 twelve-gate chronologies returned `unknown` throughout; the broad SAT route is not used as proof evidence. |
