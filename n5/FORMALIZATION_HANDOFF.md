...
### Quadratic quotient and relation map
...
These three modules are checked. The exact theorem is first stated as
...
- `N5/RankTwoSecants.lean` — the symbolic bridge from a sum of two arbitrary
  decomposable ambient forms to Hankel rank at most two (checked).
- `N5/EffectiveClassification.lean` — secant localization, synthesis of local
  Klein endpoints, and the exact 43-fiber equality and cardinality theorem
  (checked).
